...
/*   Updated: 2025/04/17 22:05:35 by kkuhn            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int nr_intersections, t_intersec **intersec_to_use, int *shape_to_use)
{
	...
	while (i < nr_intersections)
	{
		if (intersections[i].times[0] > 0 || intersections[i].times[1] > 0)
		{
			...
			else if ((intersections[i].times[0] < (*intersec_to_use)->times[0] || intersections[i].times[0] < (*intersec_to_use)->times[1]) || 
					(intersections[i].times[1] < (*intersec_to_use)->times[0] || intersections[i].times[1] < (*intersec_to_use)->times[1]))
			{
				*intersec_to_use = &intersections[i];
				*shape_to_use = intersections[i].object_id;
			}
...
t_xyzvektor	color_at(t_world *world, t_ray ray)
{
	...
	printf("%d ", shape_to_use);
	if (!intersec_to_use)
		return (set_black());
	printf("%d\n", intersec_to_use->object_id);
